...
- **`w:history` on a hyperlink is now read from and written to the source instead of being hardcoded per code path.** The reader never looked at it, and the writer's two hyperlink branches disagreed with each other: the internal-anchor path always wrote `w:history="1"`, the external-relationship path always omitted it. `w:history` is `ST_OnOff`, where an explicit `"0"` is a real, distinct value from the attribute being absent — the same explicit-versus-unset shape `w:caps` had earlier in this release — so `internal/xml.Hyperlink.History` is now `*string` rather than a bare string with `omitempty`, and a hydrated value is reused verbatim on resave instead of falling back to the old per-branch default.
- **A bookmark now survives opening and resaving a document, when its `w:bookmarkStart` and `w:bookmarkEnd` both fall in the same paragraph (#101).** The reader never hydrated bookmarks at all — `populateParagraph`'s child-element switch had no case for them — so every `REF` field's target, internal hyperlink anchor, and Word's own `_Ref…`/`_GoBack` bookmarks vanished on the next save. `core.paragraph` holds one `(id, name)` pair, the same shape the serializer already emitted a bookmark from for TOC headings, so a bookmark whose end falls in a *different* paragraph, or one that hangs directly off `w:body`, still has nowhere to live and is dropped — unchanged from before this existed. Fixed alongside, since a naive hydration would have collided with either:
