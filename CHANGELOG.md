...
- **`w:tblW` accepts decimal syntax such as `9360.0`** (#118). Real documents emitted by tolerant editors open again. Decimal measurements are truncated toward zero before the model validates them; malformed, non-finite, and overflowing values are rejected.
...
- **`domain.Header`/`domain.Footer` can now hold tables, not just paragraphs.** Tables in headers/footers are a common Word pattern (logo + address + page-number letterheads) that was previously completely unmodeled: `header.Paragraphs()` returned 0 for a table-only header, and a table there survived a resave only incidentally, via preserved bytes. Three new methods on both interfaces:
  ...
  `OpenDocument`/`OpenDocumentFromBytes`/`OpenDocumentFromReader` now hydrate header/footer tables into the domain model instead of silently dropping them (best-effort: a table docxgo can't represent, e.g. a grid wider than 63 columns, is skipped rather than failing the whole document — the same tolerance header/footer parts already have for other malformed content). `pkg/template`'s `ReplaceText`/`MergeTemplate`/`FindPlaceholders` now reach placeholders inside header/footer table cells. The CLI/RPC `headers`/`footers` content arrays accept `{"type": "table", ...}` items (previously silently decoded as an empty paragraph).

  **Known limitations:** the remaining table properties (`w:tblInd`, `w:tblCellMar`, `w:tblLayout`, `w:tblLook`, table-level `w:shd`, `w:trPr/w:cantSplit`, `w:trPr/w:tblHeader`) are still not hydrated from an existing document — the same gap body tables have. Style, width, alignment, table borders, row height and cell width/borders/shading/vertical alignment *are*, as of the table-property entry under Fixed, below. A nested table inside a table cell is still dropped on read, in both the body and a header/footer. A header/footer table in a document opened via `OpenDocument` is written back from preserved bytes on an untouched resave — by design, and only while untouched; edit it and the part is regenerated from the model (see the header regeneration entry under Fixed, below).
- **Images and hyperlinks in a header or footer now produce a valid package.** Previously an image placed in *any* header/footer paragraph — inside a table cell or not — produced a file Microsoft Word refuses to open outright ("Word experienced an error trying to open the file"), silently and with no guard, while a hyperlink there was rejected up front with an error. Two independent defects, both fixed here:
  - **`<w:hdr>`/`<w:ftr>` now declare `xmlns:wp`.** Every `<w:drawing>` wrapper element (`wp:inline`, `wp:extent`, `wp:docPr`, …) carries that prefix, and `internal/xml.Header`/`Footer` never declared it, so a header holding an image was an undeclared-prefix error. (`w:document` has always declared it.) Note this corrects an earlier entry in this same release, which claimed `xmlns:a` and `xmlns:pic` were missing too — they are not: `a:graphic` and `pic:pic` self-declare their prefixes on the drawing subtree, so `wp` was the only one a containing part had to supply.
  - **Each header and footer now owns its relationships.** A header is its own OPC part and cannot resolve an `r:id` declared in `word/_rels/document.xml.rels`, which is where every header relationship used to be minted. Each `domain.Header`/`domain.Footer` now carries its own relationship manager, written out as `word/_rels/headerN.xml.rels` / `footerN.xml.rels`, with its own ID sequence starting at `rId1`. The `w:headerReference`/`w:footerReference` IDs are unaffected — those are genuinely document-level and still come from the document's manager.

  Verified end to end with `DocxValidator` (the pre-fix package fails schema validation with `'wp' is an undeclared prefix` on `/word/header1.xml`; the fixed one passes) and by opening both in Microsoft Word.

  **Caveat, unchanged by this and not specific to hyperlinks or images:** on a document opened via `OpenDocument` whose headers were preserved for round-trip fidelity, `WriteTo` writes those parts verbatim, so *no* in-memory header edit reaches the saved file — `SetText` included. `Document.HasPreservedHeadersOrFooters` reports that case, and `pkg/template` already uses it to skip such paragraphs.
- **Header/footer relationship parts (`word/_rels/headerN.xml.rels`, `word/_rels/footerN.xml.rels`) are now preserved through their own dedicated path instead of the opaque, unrelated-parts bucket.** Previously these files survived a round-trip only incidentally, as part of `PreservedParts.Additional` — indistinguishable from any other unrecognized part in the package. They're now read into `Package.HeaderRels`/`FooterRels`, threaded through `core.PreservedParts.HeaderRels`/`FooterRels`, and written back verbatim on an untouched resave. This is the plumbing the per-part relationship support above is built on; a preserved part always wins over a generated one of the same name, so an opened document's rels still round-trip byte-for-byte.
- **`domain.Table.SetBorders` / `Borders`** — set the borders drawn around and inside a table as a whole (`w:tblBorders`), as opposed to `TableCell.SetBorders`, which only ever reaches one cell's four sides.
  - `SetBorders(borders domain.TableLevelBorders) error` / `Borders() domain.TableLevelBorders` — reuses the existing `TableLevelBorders` shape (the four outer sides plus `InsideH`/`InsideV`) that table *styles* already used.
  - `internal/xml.TableProperties.Borders` had existed for releases but nothing ever populated it outside the `styles.xml` path, so a table carrying its own hand-drawn borders had nowhere to put them and the element was never emitted.
  - Opening an existing `.docx` now hydrates `Borders()` from `w:tblPr/w:tblBorders`. Without this, a table that draws its own borders instead of referencing a style still came back looking borderless even after the `tblStyle` fix earlier in this release.
...
- **`AddHyperlink("#anchor", text)` now produces a working internal link.** Previously it unconditionally minted an `External` relationship whose target was the literal `"#anchor"` string. The serializer already resolves an internal link straight from the url, so the fix above surfaced a pre-existing bug on this path (and on the reader's own hydration of a real Word document's internal link) as a visible orphaned relationship; `run.AddField` no longer mints one when the hyperlink field's url starts with `#`. The target of an internal link now survives being read back from an existing `.docx` when its bookmark's start and end fall in the same paragraph — see bookmark hydration, below — which is the shape `AddHyperlink`'s own anchor links use.
...
- **An edit to a header or footer on an opened document now reaches the saved file.** `WriteTo` was all-or-nothing: a single preserved header discarded the entire generated map, so *every* in-memory header/footer edit on a document opened via `OpenDocument`/`OpenDocumentFromBytes`/`OpenDocumentFromReader` was silently dropped on save — `SetText`, `AddParagraph`, `AddTable`, a whole new header, all of it. It is now a per-name merge: a part the caller edited is written from the model, and every part they did not touch still goes back to the file byte-for-byte.

  "Edited" is decided by comparing each part's serialized form against a snapshot taken right after the document was hydrated, the same content-comparison shape `docRelsNeedsRegeneration` already used for `word/_rels/document.xml.rels`. Deep edits are caught for free: a `run.SetText` five levels down inside a header table cell changes the serialization, so the part is regenerated without any of the objects in between having to know they are in a header.

  Fixed alongside, because each is only reachable once headers regenerate:
  - **A header added to an opened document was handed a part name that was already taken.** The `headerN.xml` counter starts at zero for an opened document, so a new header on a file that already had `header1.xml` was assigned `header1.xml` too — two relationships pointing at one part name, and the new header's content silently discarded when they collapsed into a single archive entry. Names are now allocated past the ones the model and the preserved package already use.
  - **`[Content_Types].xml` got no `Override` for a newly added header.** It is written back verbatim on a round-trip, which is correct while the part list cannot change; a part with no declared content type makes the whole package invalid. The preserved bytes are now parsed and amended with only the missing entries, and returned untouched when nothing is missing.
  - **A header target of `/word/header1.xml` produced the entry name `word//word/header1.xml`.** A header's target comes from the source file's own rels, where the absolute form is as legal as the bare one; the writer prepended `word/` blindly. Unreachable before only because the generated branch never ran for an opened document. A target naming a *subdirectory* (`headers/header1.xml`) was wrong in the other direction once that was normalized by base name: the part was written to `word/header1.xml`, which nothing references, while the real one still went back from its preserved bytes — a junk part in the package and the edit lost. Targets are now resolved rather than collapsed: package-absolute ones name the entry directly, the rest resolve under `word/`, and the writer's resolution is what `internal/core` keys its snapshots by, so the two cannot drift apart.
  - **A regenerated part's relationships now come from the model, not the preserved `.rels`.** Pairing new content with the old relationships would leave `r:id`s pointing at nothing — the same dangling-`r:id` package #101 was about, one part further down. Exactly one of the two is written, never both.
  - **A hydrated hyperlink in a header is now declared in that header's own `.rels`.** A hydrated *image* is attached through `AttachHydratedImageToRun`, which registers its relationship into the owning part's manager; a hydrated hyperlink kept the source's `r:id` on the field and registered nothing. In the body that is harmless — the document's manager is loaded with every source relationship at hydration — but a header owns a separate manager that starts empty, so a regenerated header emitted `<w:hyperlink r:id="rId1">` with **no** `word/_rels/header1.xml.rels` written at all.
  - **An untouched header is no longer regenerated because the *body* holds an image.** `wp:docPr` ids came from one counter shared across the whole save, and `WriteTo` serializes the body before the header parts while the snapshot serializes only the parts — so the same untouched header serialized differently in the two passes and read as edited, quietly losing whatever the reader could not model from it. Each header and footer now numbers its drawings from 1, independently of the body and of every other part, which is also the right scope for an id in a separate OPC part. Making the snapshot mirror the write order would not have been enough: the caller can change the body between opening the document and saving it.
  - **A header or footer's own `.rels` is no longer preserved as though it were a header.** The reader claimed parts by literal path prefix, and `word/header` is a prefix of `word/headers/` — so the relationships file of a header stored in a subdirectory matched the header **content** test and was kept as a phantom header. The saved package then carried two entries under that one name, the preserved copy and the real one; `archive/zip` accepts a duplicate silently and leaves the reader to pick, and the stale copy can be the one missing a newly added `r:id`. This happened on an **untouched** resave, not only an edited one. Header and footer rels are now recognized by shape — a `_rels` directory, a `.rels` suffix, and an owner named `headerN.xml`/`footerN.xml` — which is also what lets both `word/_rels/header1.xml.rels` and `word/headers/_rels/header1.xml.rels` land in the same bucket, and that check now runs before the content prefixes rather than after.
  - **Header and footer part names are now handed out deterministically.** A section's headers live in a map, and ranging it gave a different header `header1.xml` from run to run of the same program. Each save was internally consistent so nothing broke outright, but two saves of one document differed, and the per-name merge is keyed on exactly that filename.
- **The first image added to an opened document that had none is now declared in `[Content_Types].xml`.** The media part is written either way, so without a `Default` for its extension the package declares a part it cannot type and Word offers to repair it. Unlike the entries above this is not a consequence of regenerating headers — it reproduces on every released version through `OpenDocument` + `AddImage` — but `amendContentTypes`, added here, is the function that owns amending the preserved part, so it is where the gap closes.
- **A table's own layout — widths, alignment, borders, row heights, cell shading — is no longer dropped when a `.docx` is opened and resaved (#102).** The `tblStyle` fix above recovered the borders a table *inherits* from a named style; this recovers everything a table states for itself. The reader hydrated a table's style reference and its merge attributes (`w:gridSpan`, `w:vMerge`) and nothing else, so a hand-laid-out table came back as an unstyled auto-width grid: on the reporter's own attachment, two columns explicitly sized 918 and 1111 twips (in both `w:tcW` and `w:tblGrid`) collapsed to `auto` on the next save, and Word re-split them evenly. Row properties were the starkest case — `w:trPr` was not read at all, only a row's `w:tc` children were. Now hydrated: `tblPr/tblW`, `tblPr/jc`, `tblPr/tblBorders`, `trPr/trHeight`, `tcPr/tcW`, `tcPr/tcBorders`, `tcPr/shd`, `tcPr/vAlign`. Except for `tblBorders` (new domain API, see Added) this was purely a reader gap: the domain setters and the serializer already handled every one of these.
- **`w:tblPr` and `w:tcPr` no longer emit their children out of schema order.** `CT_TblPrBase` and `CT_TcPrBase` are `xsd:sequence`, so child order is part of the schema, not a formatting preference. A centre-aligned table wrote `w:jc` *after* `w:tblLook`, and a cell with both shading and a vertical alignment wrote `w:vAlign` *before* `w:tcBorders`/`w:shd` — the Open XML SDK rejects both ("The element has unexpected child element ..."). This was reachable from the plain public API (`Table.SetAlignment`, `TableCell.SetVerticalAlignment` + `SetShading`) on a freshly created document, with no round-trip involved; it went unnoticed because no test validated a table against the schema and Word is lenient about it. Fixed by reordering the struct fields, which is what decides document order — this package has no custom `MarshalXML`.
- **A table whose first row is a merged full-width cell no longer gets invented column widths.** `serializeGrid` derives `w:gridCol` widths from row 0's cell widths, splitting a spanned cell's width evenly across the columns it covers. That guess only became reachable in practice once cell widths were hydrated (above): a table led by a single merged title cell would have had that one width divided equally across columns the source described as unequal. The grid is now taken from the first row that maps one-to-one onto it — one unspanned cell per column — and the even split remains only as the fallback for a table where no row does.
...
- **A hyperlink built from several runs with different formatting no longer serializes as one `<w:hyperlink>` element per run.** `expandRunWithFields` only ever sees one run at a time, so a link with, say, plain text followed by a bold word expanded to two sibling `<w:hyperlink>` elements pointing at the same target — something Word itself never produces for a single link. A post-pass over the paragraph's serialized elements now folds adjacent hyperlinks back into one when their target (`r:id`, `w:anchor`, and `w:history` together) matches; reading a document already flattens one `<w:hyperlink>` with N runs into N domain runs each carrying the same target, so the two directions round-trip through each other. Drive-by hardening in the same code path: the run-style assignment there now only applies `rStyle="Hyperlink"` if the run doesn't already carry a character style, though `domain.Run` has no character-style representation of its own today, so nothing currently reaches that branch with one set — this guards a future run type that does, not a present loss.
- **`w:history` on a hyperlink is now read from and written to the source instead of being hardcoded per code path.** The reader never looked at it, and the writer's two hyperlink branches disagreed with each other: the internal-anchor path always wrote `w:history="1"`, the external-relationship path always omitted it. `w:history` is `ST_OnOff`, where an explicit `"0"` is a real, distinct value from the attribute being absent — the same explicit-versus-unset shape `w:caps` had earlier in this release — so `internal/xml.Hyperlink.History` is now `*string` rather than a bare string with `omitempty`. The first pass at this fix left the internal-anchor path's old unconditional `"1"` in place as a fallback for whenever the reader hadn't set a value — which meant a link hydrated from a source that omitted `w:history` entirely (a common, valid shape, not just the explicit-`"0"` one) still came back with an invented `w:history="1"`. The default now lives where "brand new link" is actually unambiguous — `NewHyperlinkField`, called only by the public `AddHyperlink`, never by hydration — so a genuinely new `#anchor` link still defaults to `"1"`, while a hydrated one reuses exactly what the source had, including nothing at all.
- **A bookmark now survives opening and resaving a document, when its `w:bookmarkStart` and `w:bookmarkEnd` both fall in the same paragraph *and* wrap that paragraph's entire content (#101).** The reader never hydrated bookmarks at all — `populateParagraph`'s child-element switch had no case for them — so every `REF` field's target, internal hyperlink anchor, and Word's own `_Ref…`/`_GoBack` bookmarks vanished on the next save. `core.paragraph` holds one `(id, name)` pair, the same shape the serializer already emitted a bookmark from for TOC headings, and re-emits it as `w:bookmarkStart` at the very beginning of the paragraph and `w:bookmarkEnd` at the very end — so a bookmark whose end falls in a *different* paragraph, or one that hangs directly off `w:body`, still has nowhere to live and is dropped, unchanged from before this existed; and a bookmark that in the source wrapped only part of a run (e.g. `"target"` inside `"prefix target suffix"`) is *also* dropped rather than hydrated, because re-emitting it at the paragraph's boundaries would silently widen it to cover text it never bookmarked, corrupting whatever `REF` field pointed at it. Representing a partial-run bookmark correctly needs per-run position tracking `core.paragraph`'s single `(id, name)` pair doesn't have. Fixed alongside, since a naive hydration would have collided with either:
  - **`generateHeadingBookmarks` no longer overwrites a bookmark that came from the source.** It ran on every `WriteTo`, including a pure round trip, and unconditionally replaced any bookmark on a paragraph styled `Heading*` — so hydrating one directly into a heading paragraph, without this, would have had it destroyed and renumbered on the very next save. A paragraph now tracks whether its bookmark was hydrated versus generated (or set through the existing public `SetBookmark`), and the heading-numbering pass skips the hydrated ones.
  - **Its numbering no longer always starts at 0.** A freshly generated `_Toc0` could carry the same numeric `w:id` as an unrelated bookmark the source already had elsewhere in the document — legal-looking output that is actually two elements sharing one ID. The document now tracks the highest bookmark ID it hydrated and starts its own numbering above it. `manager.IDGenerator.NextBookmarkID`, an unrelated, dead, and separately wrong bookmark-ID generator (zero callers repo-wide; it returned `"bm1"` where `w:id` is `ST_DecimalNumber`), is removed rather than pressed into this role.
- **A theme-linked cell shading now keeps following the document's theme after being opened and resaved, instead of freezing into a flat colour (or, if the source cached no fallback, being dropped outright).** A producer commonly writes the resolved colour into `w:fill`/`w:color` as a fallback for a consumer that doesn't resolve themes, alongside `w:themeFill`/`w:themeColor` itself — but per [MS-OE376](https://learn.microsoft.com/en-us/openspecs/office_standards/ms-oe376/c7a6a5fd-538c-4a77-8cbb-0f447298dace) the theme reference is the primary colour and the cached fallback is only consulted when the reference is absent, so a producer is free to write `w:themeFill` alone with nothing to fall back to — and that link must still be kept. `SetShading`'s `domain.Color` return value can't carry that reference itself: it's a bare, public, comparable `{R, G, B}` struct compared with `!=` as a sentinel throughout this package and the serializer, so extending it would be a breaking change. `TableCell` now caches a hydrated theme reference internally (readable only via a structural type assertion, the same shape `CapsSet`/`BookmarkID` already use for state `domain.*` interfaces can't express) and the writer re-emits it alongside the cached colour when there is one — a theme-only cell (no cached fallback in the source) now emits `<w:shd w:val="clear" w:themeFill="..."/>` with no `w:fill`, rather than either fabricating a colour or dropping the shading entirely. A `w:val="solid"` source's `w:themeColor` normalizes onto the same cached slot as a `w:val="clear"` source's `w:themeFill` — an intentional simplification, matching the existing `clear`+`w:fill` normalization the plain colour already had (see "is re-emitted as the equivalent `clear` fill", above): the two patterns produce the same visible colour, so there's no need for the domain to track which one a theme link arrived paired with. Calling `SetShading` explicitly clears any cached theme reference, since a caller-supplied colour has nothing to do with the source's theme.
- **An explicitly white-shaded table cell (`<w:shd w:fill="FFFFFF">`) is no longer indistinguishable from a cell nobody touched.** `domain.ColorWhite` doubles as the zero value every new cell starts with, and the serializer suppressed any shading equal to it outright — the same explicit-versus-unset shape `w:caps` had earlier in this release. `TableCell` now tracks whether `SetShading` was ever called, and the serializer emits `w:shd` for an explicitly white cell same as any other colour.
...
- **`AddHyperlink` briefly returned an error on a header or footer paragraph during this release cycle; it no longer does.** The rejection was a stopgap for the missing per-part relationships file, which now exists (see Fixed, above), so the call works there. Relative to the last *published* release the net effect is that a header hyperlink went from producing an orphaned relationship to producing a working link — no released behavior was removed.
- **`pkg/template`'s `ReplaceText` and `MergeTemplate` no longer skip header and footer matches on a round-tripped document.** They skipped them wholesale because `WriteTo` wrote every preserved header back verbatim and the edit could never reach the file: `ReplaceText` counted those matches in `Skipped`, and `MergeTemplate` reported the placeholder through `missingKeys` (so `StrictMode` failed). Both now replace and fill normally, because the edit does reach the file. If you relied on the old behaviour — a strict-mode merge failing rather than touching a preserved header — check for it with `Document.HasPreservedHeadersOrFooters` before calling.
- **`domain.Table` interface gained two methods** (`SetBorders`, `Borders`, above). If you implement `domain.Table` directly with your own type — most commonly a hand-written test double — you'll need to add both; embedding `domain.Table` in your type is unaffected, since the new methods are promoted automatically. No exported docxgo API accepts a `domain.Table` from a caller (only returns one). Note this reasoning does **not** hold for `domain.Paragraph`, which `pkg/template.ConsolidateRuns` does accept — don't copy this clause for a future `Paragraph` change.
...
- **When two sections reference the same header part, only the first section's copy is written.** The reader hydrates a separate `domain.Header` per section, but serialization keys parts by target and keeps the first it reaches, so editing the object belonging to a later section changes nothing: the part never diverges from its snapshot, and the preserved bytes go back untouched. Not a regression — before this release *no* header edit on an opened document reached the file — but it is the one case where an edit still silently does not, so it is worth stating rather than leaving to be discovered. Edit the first section's header for that part and it lands normally.
- **A regenerated header or footer is rebuilt from what docxgo can model, so anything the reader does not understand is dropped from that part.** Regeneration is opt-in by mutation, which bounds the damage — a part nobody edited is still byte-for-byte the original — but the trade is real and worth stating plainly: edit a header holding a content control, a field, or a nested table, and that content is gone from the saved header even though the edit itself lands. This is why the merge is per name rather than "regenerate everything": the table-property work earlier in this release makes a regenerated table much closer to its source, but it does not make regeneration lossless. `Document.HasPreservedHeadersOrFooters` reports the round-tripped case if you want to decide differently.
- **Table property hydration is lossy in eight specific, deliberate ways.** Each is a place where the domain model has no way to express what the source said, and inventing one would be worse than dropping it:
  - **A non-`dxa` cell width is not hydrated at all.** `TableCell.SetWidth` takes a bare twip count with no type, and the serializer writes `w:type="dxa"` for any positive width, so hydrating `<w:tcW w:type="pct" w:w="2500"/>` — half the table — would rewrite it as a fixed 2500 twips (1.7 inches). Wrong, not merely lossy. Left alone it degrades to `auto`, which still lays out sensibly. Table-level `w:tblW` is unaffected: `domain.TableWidth` carries its own type, so `pct` and `auto` round-trip as themselves.
  - **A row's `w:hRule` is dropped.** `TableRow.SetHeight` holds a bare twip count and the serializer always writes `hRule="atLeast"`, so `exact` and `auto` both come back as a minimum height.
  - **Cell shading is only read when it resolves to one flat colour.** A real pattern fill (`w:val` other than `clear`/`solid`) blends `w:color` and `w:fill`, and an `auto` value names no concrete colour, so neither has a single colour to map onto `SetShading`.
  - **A `w:val="solid"` shading is re-emitted as the equivalent `clear` fill.** `w:shd` paints a `w:val` pattern in `w:color` over a `w:fill` background, so a `solid` pattern's visible colour is its `w:color` (the foreground covers the background completely) while a `clear` one's is its `w:fill`. Both are hydrated — from the right attribute each — but the domain holds only the resulting colour, so the writer emits every shading as `clear` + `w:fill`. Visually identical; the original `w:val` and the hidden second colour are gone.
  - **An explicit `<w:… w:val="none"/>` border is indistinguishable from an absent one.** `domain.BorderNone` is the zero value, so a border explicitly set to none in order to *suppress* one a table style would otherwise supply is dropped and the style's border returns. Same explicit-versus-unset problem `w:caps` had earlier in this release, unsolved here.
  - **`ST_Border` styles outside the seven the domain models are approximated as a plain single line.** The enum has on the order of 180 members; keeping the border visible is closer to the source than erasing a line the author drew, but a wavy or thick-thin border comes back plain.
  - **`w:tblGrid` itself is still not read.** Column widths are recovered only from the cells (`w:tcW`), and `w:gridCol` widths are re-derived from them on write, so a table that declares its widths *solely* in `w:tblGrid` — legal, and what some producers emit — still comes back with an unsized grid. The reporter's own attachment states its widths in both places, which is why the round-trip above recovers them. Reading `tblGrid` directly would mean writing those widths onto row-0's cells, since the domain has no per-column width independent of a cell — the same "wrong, not merely lossy" trade as the non-`dxa` cell width above, so it is left alone rather than approximated. Pre-existing; unchanged by this release.
  - **`w:tblLook` is still written as a hardcoded `04A0`,** clobbering whatever the source carried (band and first-row/column hints). Unchanged by this release, and now the most visible remaining `tblPr` loss.
