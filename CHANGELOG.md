...
- **Table property hydration is lossy in six specific, deliberate ways.** Each is a place where the domain model has no way to express what the source said, and inventing one would be worse than dropping it:
  ...
  - **Cell shading is only read when it is a plain solid fill.** A pattern fill (`w:val` other than `clear`/`solid`), `w:fill="auto"`, and `w:themeFill` have no single colour to map onto `SetShading`. Separately, the serializer suppresses shading equal to `ColorWhite`, so an explicitly white-shaded cell is still unrepresentable.
