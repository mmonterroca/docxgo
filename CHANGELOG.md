...
- **Table property hydration is lossy in nine specific, deliberate ways.** Each is a place where the domain model has no way to express what the source said, and inventing one would be worse than dropping it:
  ...
  - **Cell shading is only read when it resolves to one flat colour.** A real pattern fill (`w:val` other than `clear`/`solid`) blends `w:color` and `w:fill`, and an `auto` value names no concrete colour, so neither has a single colour to map onto `SetShading`. Separately, the serializer suppresses shading equal to `ColorWhite`, so an explicitly white-shaded cell is still unrepresentable.
  - **A theme-linked shading round-trips as a flat colour and stops following the theme.** When `w:themeFill`/`w:themeColor` is present it, not `w:fill`/`w:color`, is what a theme-aware consumer resolves the colour from — but producers write the resolved colour into the plain attribute as a fallback for consumers that do not, and that cached value is what gets hydrated. The domain has no way to hold a theme reference, so the saved cell keeps its current colour and no longer changes when the document's theme does. Dropping the shading instead was considered and rejected: a table is always rebuilt from the model on save, so declining to hydrate would not preserve the link, it would delete the shading and return the cell white. A `w:themeFill` with **no** `w:fill` alongside it still hydrates nothing, because then there is no cached colour to keep.
