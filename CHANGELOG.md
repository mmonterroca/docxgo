...
- **A header or footer that lives in a subdirectory *and* owns relationships still loses them.** The content part itself now round-trips and regenerates correctly wherever the target points (see Fixed, above), but the reader claims preserved parts by literal path prefix, and `word/headers/_rels/header1.xml.rels` matches the header **content** prefix `word/header` — so it is kept as though it were a header rather than as that header's relationships. Tightening that classification touches every part kind the reader recognizes, so it is left for its own change. Only reachable on a source document that both places its headers outside `word/` and gives them relationships of their own; a header in `word/` is unaffected either way.
