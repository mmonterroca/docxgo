...
- **A theme-linked cell shading now keeps following the document's theme after being opened and resaved, instead of freezing into a flat colour.** When `w:themeFill`/`w:themeColor` is present alongside `w:fill`/`w:color`, the plain attribute is a producer-written fallback for a consumer that doesn't resolve themes — a theme-aware one, Word included, actually resolves the colour from the theme reference. `SetShading`'s `domain.Color` return value can't carry that reference itself: it's a bare, public, comparable `{R, G, B}` struct compared with `!=` as a sentinel throughout this package and the serializer, so extending it would be a breaking change. `TableCell` now caches a hydrated theme reference internally (readable only via a structural type assertion, the same shape `CapsSet`/`BookmarkID` already use for state `domain.*` interfaces can't express) and the writer re-emits it alongside the cached colour. A `w:val="solid"` source's `w:themeColor` normalizes onto the same cached slot as a `w:val="clear"` source's `w:themeFill` — an intentional simplification, matching the existing `clear`+`w:fill` normalization the plain colour already had (see "is re-emitted as the equivalent `clear` fill", above): the two patterns produce the same visible colour, so there's no need for the domain to track which one a theme link arrived paired with. Calling `SetShading` explicitly clears any cached theme reference, since a caller-supplied colour has nothing to do with the source's theme.
